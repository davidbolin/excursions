...
test_that("stack extraction", {
  if (require.nowarnings("INLA")) {
    ...
    expect_equal(config$mu,mu,tolerance=1e-7)
    ...
    expect_equal(config$vars,vars,tolerance=1e-7)
  }
})
...
test_that("Contourmap.inla, P measures", {
  if (require.nowarnings("INLA")) {
    ...
    expect_equal(res1$P1,0.7732031,tolerance=1e-7)
    expect_equal(res1$P2,0.6558154,tolerance=1e-7)
  }
})
